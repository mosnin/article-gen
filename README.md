...
- Scheduler page for per-blog queued/calendar content automation (interval and calendar modes).
...
- `blog_schedulers`: per-blog interval scheduler configuration.
- `scheduled_posts`: queued and calendar-scheduled topics waiting for automation.
...
## Scheduler automation (Edge Function)

The Supabase Edge Function scaffold is located at `supabase/functions/scheduler-run/index.ts`.

- It claims due calendar posts and interval queue items per blog.
- It advances `next_run_at` per active blog scheduler.
- It marks claimed items as `processing` so a generation/publish worker can execute them.

This gives you the scheduling backbone for hourly/per-blog automation.
