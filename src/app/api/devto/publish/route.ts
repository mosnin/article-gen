...
import { publishToDevto } from "@/lib/publish";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { articleId, accountId, tags, published, canonicalUrl } =
      (await req.json()) as {
        articleId: string;
        accountId?: string;
        tags?: string[];
    ...
    const result = await publishToDevto(
      ...
      { tags, published, canonicalUrl },
    );

    if (!result.success) {
      const msg = result.error ?? "Failed to publish to Dev.to";
      const status = /authentication failed/i.test(msg)
        ? 401
        : /not found/i.test(msg)
          ? 404
          : /No Dev\.to account connected/i.test(msg)
            ? 400
            : 500;
