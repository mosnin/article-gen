...
export default function ConnectedBlogsPage() {
  ...
  const addBlog = () => {
    if (blogs.length >= MAX_BLOGS) return;
    setBlogs((prev) => [...prev, {
      id: crypto.randomUUID(),
      name: "",
      url: "",
  ...
  return (
    <AppShell title="Connected Blogs" onSignOut={handleLogout}>
        {/* WordPress Blogs */}
        <section style={{ marginBottom: 40 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
            <div>
              <h2 style={{ fontSize: 20, fontWeight: 700, marginBottom: 2 }}>Connected Blogs</h2>
              <p style={{ fontSize: 13, color: "var(--muted)" }}>Connect up to 5 WordPress sites for publishing. Articles won&apos;t auto-publish &mdash; you choose when to publish.</p>
            </div>
          </div>

          <div className="mb-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            <div style={{ background: "var(--card)", border: "1px solid var(--card-border)", borderRadius: 12, padding: 14 }}>
              <div style={{ fontSize: 12, color: "var(--muted)", marginBottom: 4 }}>Connected</div>
              <div style={{ fontSize: 20, fontWeight: 700 }}>{blogs.length} / {MAX_BLOGS}</div>
            </div>
            <div style={{ background: "var(--card)", border: "1px solid var(--card-border)", borderRadius: 12, padding: 14 }}>
              <div style={{ fontSize: 12, color: "var(--muted)", marginBottom: 4 }}>Primary Blog</div>
              <div style={{ fontSize: 14, fontWeight: 600 }}>{blogs[0]?.name || blogs[0]?.url || "Not set"}</div>
            </div>
            <div style={{ background: "var(--card)", border: "1px solid var(--card-border)", borderRadius: 12, padding: 14 }}>
              <div style={{ fontSize: 12, color: "var(--muted)", marginBottom: 4 }}>Publishing</div>
              <div style={{ fontSize: 14, fontWeight: 600 }}>Manual publish per article</div>
            </div>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div key={blog.id} style={{ background: "var(--card)", border: "1px solid var(--card-border)", borderRadius: 12, overflow: "hidden" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 20px", borderBottom: "1px solid var(--card-border)", gap: 8, flexWrap: "wrap" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                    <span style={{ minHeight: 24, borderRadius: 999, background: "var(--accent)", color: "#fff", display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, padding: "0 10px" }}>
                      {idx === 0 ? "Primary" : `Blog ${idx + 1}`}
                    </span>
                    <span style={{ fontSize: 14, fontWeight: 600 }}>{blog.name || (idx === 0 ? "Primary Blog" : `Blog ${idx + 1}`)}</span>
                    {testResults[blog.id] && (
            ...
            {blogs.length < MAX_BLOGS && (
              ...
              >
                ...
                Add WordPress Blog ({blogs.length}/{MAX_BLOGS})
              </button>
            )}
          </div>
        </section>
