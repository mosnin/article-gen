...
export function StarButton({
  ...
  return (
    ...
    >
      ...
      />
      <div
        className="absolute inset-0 border-black/10 z-[4] overflow-hidden rounded-[inherit] text-white"
        style={{ borderWidth: "var(--border-width)" }}
        aria-hidden="true"
      >
        <StarBackground color={backgroundColor} />
      </div>
      <span className="z-10 relative bg-gradient-to-t from-black to-neutral-400 inline-block text-transparent bg-clip-text">
        {children}
      </span>
    </button>
  );
...
// ─── Link variant for use with Next.js Link ───────────────────────────────────

interface StarButtonLinkProps {
  children: ReactNode;
  lightWidth?: number;
  duration?: number;
  lightColor?: string;
...
export function StarButtonInner({
  children,
  lightWidth = 110,
  duration = 3,
  lightColor = "#FAFAFA",
  ...
  return (
    <div
      ...
      className={cn(
        "relative z-[3] overflow-hidden h-10 px-4 py-2 inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-3xl text-sm font-medium group/star-button",
        className,
      )}
    >
      <div
